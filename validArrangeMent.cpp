...
vector<vector<int>> validArrangement(vector<vector<int>> &pairs) {
	...
	while (i < eulerPath.size()) {
		...
		if (swapped && graph[end].size() == 1) { swap(start, end); }
	...
	if (result[0][0] != pairs[0][0]) { reverse(result.begin(), result.end()); }
...
int main() {
	...
	for (vector<int> &pairs : validPairs) {
		for (int values : pairs) { cout << values << " "; }
		cout << " ";
	}
	cout << endl;
}